...
export const metadata = { title: "ลูกหนี้ค้างชำระ — PropOS" };
...
export default async function ReceivablesPage() {
  ...
  const invoices = await prisma.accDocument.findMany({
    where: { docType: "INVOICE", status: "AWAITING_PAYMENT" },
    orderBy: { dueDate: "asc" },
    include: { contact: { select: { name: true, phone: true } } },
  });

  const rows = invoices.map((d) => {
    // no due date = treat as due on the issue date
  ...
  // group by contact, biggest debtors first
  ...
  return (
    <div className="space-y-4">
      <div>
        <h1 className="text-lg font-semibold">ลูกหนี้ค้างชำระ</h1>
        <p className="text-sm text-muted-foreground">
          ใบแจ้งหนี้สถานะรอชำระทั้งหมด {rows.length} ฉบับ รวม ฿{fmtMoney(grandTotal)}
        </p>
      </div>
